...
class Loomi_Updater {
	...
	const TRANSIENT_KEY = 'loomi_update_check';
	...
	public static function inject_update( $transient ) {
		...
		if ( ! $remote || ! is_string( $remote['version'] ?? null ) ) {
			return $transient;
		}
		...
		$local_version = LOOMI_STUDIO_VERSION;
		...
		$transient->response[ LOOMI_STUDIO_BASENAME ] = (object) [
			'id'           => self::SLUG . '/' . self::SLUG . '.php',
			'slug'         => self::SLUG,
			'plugin'       => LOOMI_STUDIO_BASENAME,
	...
	public static function clear_cache_after_upgrade( $upgrader, $hook_extra ) : void {
		...
		if ( in_array( LOOMI_STUDIO_BASENAME, $hook_extra['plugins'], true ) ) {
	...
	private static function check_remote() : ?array {
		...
		if ( is_wp_error( $response ) ) {
			set_transient( self::TRANSIENT_KEY, [], MINUTE_IN_SECONDS * 30 );
			return null;
		}
